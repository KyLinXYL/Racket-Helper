...
(define (if-fail-condition exp) (cadr exp))
(define (if-fail-alt exp) (caddr exp))
(define (analyze-if-fail exp)
  (let ((if-cond (analyze (if-fail-condition exp)))
        (if-alt (analyze (if-fail-alt exp))))
    (lambda (env succeed fail)
      (if-cond env
               (lambda (val next)
                 (succeed val fail))
               (lambda () (if-alt env succeed fail))))))


(define (all-answer? exp) (tagged-list? exp 'all-answer))
(define (all-answer-exp exp) (cadr exp))
(define (analyze-all-answer exp)
  (let ((ans (analyze (all-answer-exp exp))))
    (lambda (env succeed fail)
      (ans env (lambda (val next)
                 (display val) (newline) (next))
           (lambda () (set! isprint #f) (fail))))))
