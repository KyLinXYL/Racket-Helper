         ...
         (let ((next-inst (car text)))
           (if (symbol? next-inst)
                 (if (assoc next-inst labels)
                     (begin (set! is-good-machine #f) (receive '() '()))
                     (receive insts
...
(define (make-swap inst machine pc)
  (let ((A (get-register machine (swap-A-name inst)))
        (B (get-register machine (swap-B-name inst))))
    (lambda ()
      (let ((A-value (get-contents A))
            (B-value (get-contents B)))
        (set-contents! A B-value)
        (set-contents! B A-value)
        (advance-pc pc)))))
(define (swap-A-name inst) (cadr inst))
(define (swap-B-name inst) (caddr inst))
