...
(define (new-if-pred exp) (cadr exp))
(define (new-if-exp1 exp) (caddr exp))
(define (new-if-exp2 exp) (cadddr exp))

(define (analyze-new-if exp)
  (let((pred (analyze (new-if-pred exp)))
       (exp1 (analyze (new-if-exp1 exp)))
       (exp2 (analyze (new-if-exp2 exp))))
    (lambda (env success fail)
      (pred env
            (lambda (pred-value fail2)
              (exp1 env success fail2))
            (lambda () (exp2 env success fail))))))
